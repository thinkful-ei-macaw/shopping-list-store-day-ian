  ...
  return `
    <li class='js-item-element' data-item-id='${item.id}'>
      ...
      <div class='shopping-item-controls'>
        ...
        <input type="text" name="shopping-list-entry" class="js-shopping-list-edit-to" placeholder="edit">
      </div>
    </li>`;
};
...
function editItem(newName, index) {
  const foundItem = store.items.find(item => item.id === index);
...
function handleEditItem(){
  $('.js-shopping-list').on('click', '.js-item-edit', event => {
    const id = getItemIdFromElement(event.currentTarget);
    const newName = $('.js-shopping-list-edit-to').val();
    $('.js-shopping-list-edit-to').val('');
    editItem(newName, id);
    render();
  })
}
